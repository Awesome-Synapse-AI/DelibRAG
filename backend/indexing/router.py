...
@router.post("/index")
async def index_document(_user=Depends(require_role(UserRole.engineer, UserRole.clinician, UserRole.manager, UserRole.admin))):
...
@router.get("/sources")
async def list_sources(_user=Depends(require_role(UserRole.engineer, UserRole.clinician, UserRole.manager, UserRole.admin))):
